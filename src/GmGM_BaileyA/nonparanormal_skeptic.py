...
def nonparanormal_matrix_skeptic(
    Ys: np.ndarray
):
    """
    An implementation from the scBiGLasso paper
...
def nonparanormal_tensor_skeptic(
    ...
    """
    A generalization of the nonparanormal skeptic to tensors a la scBiGLasso

    While the code looks more complicated, it is the exact same as before!
    We just matricize the tensor along each of its axes.
    """
    if batchless:
        Ys = Ys[np.newaxis, ...]
    m, *ds = Ys.shape
